...
LATITUDE = 47.99
LONGITUDE = -81.85
RADIUS_NM = 5.0  # 5 nautical miles = 10 NM diameter
...
# Based on Cesaroni M2245 motor with 28.5 lbs loaded / 19 lbs empty rocket
# Average vertical velocity during powered flight (burn time: 4.37s)
ASCENT_RATE_MPS = 384.7  # m/s (Average during burn, conservative for AoA)
# Note: Peak velocity at burnout is 769.4 m/s, but using average for fin analysis
...
def main():
    ...
    for file_idx, input_file in enumerate(existing_files, 1):
        ...
        print(f"  Center: {LATITUDE}{degree_symbol}N, {lon_use}{degree_symbol}E")
        print(f"  Bounds: {lat_min:.2f}{degree_symbol}N to {lat_max:.2f}{degree_symbol}N")
        print(f"          {lon_min:.2f}{degree_symbol}E to {lon_max:.2f}{degree_symbol}E")
